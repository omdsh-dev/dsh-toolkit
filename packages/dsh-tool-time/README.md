...
本插件已迁移到 DSH 0.1.2-alpha.2 依赖线，并在 `local harness 0.1.2-alpha.2`（npm 私有包）的隔离 consumer 中完成全链路验证：
...
- **消费验证**：tarball 装入 DSH 0.1.2-alpha.2（npm）consumer → `dsh --profile compat --dump-config` 出现本插件 row → 工具真实注册与执行通过
- **启动方式**：`npx -p @deepseek-ai/dsh@next dsh web`（lib 生产模式；勿 `install -g` 全局安装）


## 版本适配

- **适配 DSH**: DSH 0.1.2-alpha.2（npm）（profile/bundle 插件系统）
...
将本插件作为独立 bundle 安装到 profile（DSH 0.1.2-alpha.2（npm））。本仓库位于 [omdsh-dev](https://github.com/omdsh-dev) 组织，公开可访问：
...
> 注意：patch 是 id-targeted 语义（DSH 0.1.2-alpha.2（npm））——裸 `- id:` 条目会报 `entry "xxx" not found`，必须用 `- insert:` 列表包裹。
## 已知限制
