...
This plugin has been migrated to the DSH 0.1.5-alpha.1 harness and fully verified end-to-end in an isolated consumer of `local harness 0.1.5-alpha.1`:
...
- **Consumption verification**: tarball loaded into a 0.1.5-alpha.1 consumer → `dsh --profile compat --dump-config` shows this plugin's row → the tool genuinely registers and executes
- **Launch method**: `npx -p @deepseek-ai/dsh@next dsh web` (lib production mode; don't `install -g` globally)


## Version Adaptation

- **DSH adapted**: DSH 0.1.5-alpha.1 (migration: profile/bundle plugin system)
- **bundle declaration**: `dsh.bundle` in `package.json` (patch points to `cordis.patch.yml`) + `exports` export
- **patch format**: `cordis.patch.yml` uses the `- insert:` list (the DSH 0.1.5-alpha.1 patch is id-targeted semantics; a bare `- id:` entry reports `entry not found`)
...
As of DSH 0.1.5-alpha.1, this plugin can be installed into any profile as a standalone bundle in one step (repositories live at https://github.com/omdsh-dev, public):
...
> DSH 0.1.5-alpha.1 note: the patch is id-targeted semantics — a bare `- id:` entry reports `entry "xxx" not found`; it must be wrapped in a `- insert:` list.
## Usage
...
1. **Distribution chain**: `@deepseek-ai/dsh-tools` is now published as a private npm package with DSH 0.1.5-alpha.1; the plugin installs directly via `dsh plugin add github:omdsh-dev/...` or the tarball, with no need to place it in the monorepo for workspace resolution
